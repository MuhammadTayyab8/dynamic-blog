...
const PostsPage = () => {
  ...
  const fetchPosts = async () => {
    try {
      ...
      const postsRes = await fetch(
        'https://dev-codescode.pantheonsite.io/wp-json/wp/v2/posts?categories=5&per_page=9&_=' + Date.now(),
        { cache: 'no-store' }
      );
  ...
  return (
    <div className="pr-2 md:p-6 md:pt-0  md:pr-4 pb-28 overflow-auto">
  ...
  {loading ? (
    ...
    <>
      {posts.map((post) => (
        ...
        <div
          key={post.id}
          className="border border-gray-700 bg-[#1b1f26] w-[290px] md:w-[240px] h-[450px] lg:w-[320px] p-3 rounded-2xl mt-6 cursor-pointer flex flex-col justify-between"
        >
