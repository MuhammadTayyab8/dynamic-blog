...
const PostsPage = () => {
  ...
  const fetchPosts = async () => {
    try {
      ...
      const postsRes = await fetch(
        'https://dev-codescode.pantheonsite.io/wp-json/wp/v2/posts?categories=1&_=' + Date.now(),
        { cache: 'no-store' }
      );
  ...
  return (
    <div className="md:p-6 md:pt-0  md:pr-4 pb-28">
  ...
  {loading ? (
    ...
    <>
      {posts.map((post) => (
        ...
        <div
          key={post.id}
          className="border border-gray-500 w-[290px] bg-[#1b1f26] md:w-[240px] h-[450px] lg:w-[320px] p-3 rounded-2xl mt-6 cursor-pointer flex flex-col justify-between"
        >
